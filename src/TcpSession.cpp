...
// ── Constructor ───────────────────────────────────────────────────────────
TcpSession::TcpSession(asio::ip::tcp::socket socket, const ServerConfig& config)
    : id_(nextId_++)
...
{
  asio::post(strand_,
             [self = shared_from_this(), buf = std::move(data)]() mutable {
               self->writeQueue_.push_back(std::move(buf));
               if (!self->writing_) {
                 self->doWrite();
               }
