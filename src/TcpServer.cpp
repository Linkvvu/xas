...
void TcpServer::onDisconnect(std::function<void(SessionPtr, std::error_code)> cb)
{
        ...
        [this](std::error_code ec, asio::ip::tcp::socket socket) {
            ...
            asio::post(sessionStrand_, [this, sock = std::move(socket)]() mutable {
                ...
                if (overloaded) {
                    ...
                    sock.close(closeEc);
                    if (overloadCb_) overloadCb_();
                    config_.logger->warn("max connections reached, rejected new connection");
                    return;
                }
                ...
                sess->setDisconnectCallback(
                    [this](SessionPtr s, std::error_code e) {
                        if (disconnectCb_) disconnectCb_(s, e);
                        asio::post(sessionStrand_, [this, id = s->id()] {
                            removeSession(id);
                        });
                    });

                if (errorCb_) sess->setErrorCallback(errorCb_);
                if (idleCb_)  sess->setIdleCallback(idleCb_);
