...
// ── EchoCodec ────────────────────────────────────────────────────────────────
// Treats every incoming chunk as a complete message and echoes it back verbatim.
...
template<typename T>
static bool waitFor(std::future<T>& fut,
...
// ── Test fixture ──────────────────────────────────────────────────────────────
// Each test creates its own server with a fresh port; the fixture only provides
// common teardown logic so that the server is always stopped even on failure.
...
protected:
    ...
    xas::CodecHandle<xas::Buffer> makeEchoServer(uint16_t port,
        ...
        server_ = std::make_unique<xas::TcpServer>("127.0.0.1", port, cfg);
...
// ─────────────────────────────────────────────────────────────────────────────
// 2. OnConnectCallback
//    The onConnect callback must fire once a client establishes a TCP connection.
// ─────────────────────────────────────────────────────────────────────────────
TEST_F(EchoServer, OnConnectCallback) {
    ...
    server_->onConnect([&](xas::SessionPtr sess) {
        std::call_once(once, [&] {
            connectedPromise.set_value(sess);
        });
    });
    ...
    ASSERT_TRUE(waitFor(connectedFuture)) << "onConnect callback was not called within 3 s";
